...
public class NonBlockingStartupService(
    ILogger<NonBlockingStartupService> logger, 
    IServiceScopeFactory scopeFactory,
    HttpClient httpClient) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        logger.LogInformation("NON-BLOCKING STARTUP SERVICE starting...");
        ...
        try
        {
            ...
            logger.LogWarning("STARTING REALISTIC NON-BLOCKING STARTUP CHECKS...");
            logger.LogWarning("Non-Blocking startup demonstration begins at: {StartTime}", DateTime.Now.ToString("HH:mm:ss.fff"));
            
            // STEP 1: NON-BLOCKING STARTUP HEALTH CHECKS (60 seconds total)
            var startupResult = PerformBlockingStartupHealthChecks(stoppingToken);
            
            logger.LogWarning("Realistic non-blocking startup completed");
            ...
            logger.LogWarning("ALL NON-BLOCKING OPERATIONS COMPLETE - API NOW AVAILABLE");
        ...
        {
            logger.LogError(ex, "Error in non-blocking startup service - this would prevent app from starting!");
        }
    }
